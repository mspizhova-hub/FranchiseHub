...
function saveCurrentScenario(nameInputId){
  ...
  // Тут для спрощення збережемо за замовчуванням базовий (можна розширити UI для ручного налаштування multipliers)
  // Якщо користувач натискав кнопку preset — вона зберігає останній вибір у data-current-preset
...
function renderSavedList(){
  ...
  list.forEach(s=>{
    ...
    const loadBtn = document.createElement('button'); loadBtn.textContent='Використати'; loadBtn.addEventListener('click', ()=>{
      ...
      // Для простоти — заповнюємо слот A якщо порожній, інакше B, інакше C
      // Але тут надаємо діалог вибору слоту
...
document.addEventListener('DOMContentLoaded', ()=>{
  ...
  // Наповнюємо селект
  populateFranchiseSelect('franchise_cmp');
  // Пресети
